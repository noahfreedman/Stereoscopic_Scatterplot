...
// esc opens the option menu, closes submenus.
  ...
  void MenuMain ()
		{
				if (ShowCreateLineMenu || ShowCreatePlaneMenu || ShowLoadPointsMenu || ShowGenPointsMenu || ShowOptionsMenu) {
				} else {
						...
						if (GUILayout.Button ("Create Point Data..")) {
						...
						if (GUILayout.Button ("Quit.")) {
								ShowMenu = false;
								// TODO: Application.Quit isn't working - or doesnt work in debug... try in a build
								Application.Quit ();
		...
		GUILayout.BeginHorizontal ();
				if (GUILayout.Button ("Generate")) {
					//Stage.GetComponent<RandomPoints> ().createRandomPoints (333);
